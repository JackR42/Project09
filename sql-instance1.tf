resource "azurerm_mssql_server" "project-sql-instance1" {
...
resource "azurerm_mssql_database" "project-db1" {
  ...
  depends_on = [
    azurerm_mssql_server.project-sql-instance1
  ]
  tags = {
    Subscription = var.sub-name
    Application = var.app-name
...
resource "azurerm_mssql_firewall_rule" "project-sql-instance1-fw0" {
  name = "FirewallRule-sql-instance1-fw0"
  server_id = azurerm_mssql_server.project-sql-instance1.id
  start_ip_address = "0.0.0.0"
  end_ip_address = "0.0.0.0"
}
# Create FW rule to allow access from OFFICE
resource "azurerm_mssql_firewall_rule" "project-sql-instance1-fw1" {
  name = "FirewallRule-sql-instance1-fw1"
  server_id = azurerm_mssql_server.project-sql-instance1.id
  start_ip_address = "91.205.194.1"
  end_ip_address = "91.205.194.1"
}
# Create FW rule to allow access from HOME
resource "azurerm_mssql_firewall_rule" "project-sql-instance1-fw2" {
  name = "FirewallRule-sql-instance1-fw2"
  server_id = azurerm_mssql_server.project-sql-instance1.id
